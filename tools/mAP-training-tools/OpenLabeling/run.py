...
def get_gv_path(img_path):
    img_name = os.path.basename(os.path.normpath(img_path))
    img_type = img_path.split('.')[-1]
    return groundVal_dir + img_name.replace(img_type, 'txt')
...
if not os.path.exists(groundVal_dir):
    os.makedirs(groundVal_dir)
    ...
    if point_1[0] is not -1:
        ...
        if point_2[0] is not -1:
            ...
            if args.format == 'yolo':
                ...
                print("person", xmin,ymin,xmax,ymax)
                newline = "person " + str(xmin) + " " + str(ymin) + " " + str(xmax) + " " + str(ymax)


            elif args.format == 'voc':
                line = voc_format(class_index, point_1, point_2)
            save_bb(txt_path, line)
            save_bb(gv_path, newline)
            # reset the points
            point_1 = (-1, -1)
            point_2 = (-1, -1)
        else:
